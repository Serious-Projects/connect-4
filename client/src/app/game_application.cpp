...
int app::run_game() {
    ...
    // Match, animation and score state.
    Game game;
    FallingDisc falling;
    ...
    float cursor_send_cooldown = 0;
    int latency_ms = -1;

    // Overlay panels and transient effects.
    ...
    auto sync_online = [&](const online::State& state, const online::Event* move_event = nullptr) {
        ...
        const bool empty = std::all_of(state.board.begin(), state.board.end(), [](int value) { return value == 0; });
    ...
    auto player_label = [&](Player player) -> std::string {
        ...
        return online_names[index] + (online_seat != 0 && static_cast<int>(player) == online_seat ? " (You)" : "");
    };
    ...
    auto celebrate = [&] {
        ...
        for (int i = 0; i < 180; ++i) {
            ...
            confetti.push_back(
                {{x, static_cast<float>(GetRandomValue(-180, -10))},
                 {static_cast<float>(GetRandomValue(-90, 90)), static_cast<float>(GetRandomValue(100, 420))},
                 0,
    ...
    while (!WindowShouldClose()) {
        ...
        // --- Networking: keep-alive, pending attempt, relay events ---

        if (!online_attempt_pending) online_client.keep_alive();

        if (online_attempt_pending && online_attempt.wait_for(std::chrono::seconds(0)) == std::future_status::ready) {
        ...
        for (const auto& event : online_client.poll()) {
            if (event.type == online::Event::Type::welcome) {
                ...
                if (event.state.rematch_votes > 0 && game.over())
                    online_status = "Rematch votes: " + std::to_string(event.state.rematch_votes) + " of 2.";
                ...
                online_status = event.rival_connected ? "Both players connected. Game on!"
                ...
                if (online_mode && !online_room.empty() && !online_token.empty()) reconnect_timer = 1.0F;
            }
        }

        if (online_mode && !online_client.connected() && !online_attempt_pending && reconnect_timer >= 0) {
            reconnect_timer -= dt;
            if (reconnect_timer <= 0) {
                ...
                begin_online_attempt([&online_client, room, name, token] {
                    ...
                    return OnlineAttempt{connected, connected ? "Connection restored." : "Could not reconnect."};
                });
            }
        }
        ...
        const bool replay_ready =
            online_mode ? !available_replay.empty()
                        : outcome_announced && !falling.active && pending_falls.empty() && !local_replay.empty();

        const bool current_round_visually_ready =
            !game.over() || (outcome_announced && !falling.active && pending_falls.empty());

        if (!online_lobby && IsKeyPressed(KEY_P) && (replay_active || (replay_ready && current_round_visually_ready))) {
            if (replay_active)
                stop_replay();
            else
                start_replay();
        ...
        if (sound_panel) {
            ...
            if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && CheckCollisionPointRec(mouse, mute_button)) {
        ...
        if (reaction_panel) {
            ...
            for (int index = 0; index < 4; ++index) {
                ...
                if (IsKeyPressed(KEY_ONE + index) ||
                    (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && CheckCollisionPointRec(mouse, button)))
                    selected_reaction = index;
            }

            if (selected_reaction >= 0) {
                if (!online_attempt_pending) online_client.send_reaction(ids[selected_reaction]);
                reaction_panel = false;
            }
        }

        // --- Input: replay playback controls ---
        ...
        if (online_mode && !online_lobby && !online_room.empty() &&
            (IsKeyPressed(KEY_C) ||
             (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && CheckCollisionPointRec(mouse, room_code_card)))) {
            SetClipboardText(online_room.c_str());
            online_status = "Room code copied - paste it to your friend.";
        }

        // --- Input: lobby text fields, or board play when in a game ---
        ...
        if (online_lobby) {
            ...
            if (!name_field_active && control && IsKeyPressed(KEY_V)) {
                if (const char* clipboard = GetClipboardText()) {
                    ...
                    if (room_input.empty())
                        online_status = "No valid six-character room code was found in the clipboard.";
                }
            }
            ...
            if (!control) {
                ...
                while (character > 0) {
                    const bool valid_name = character >= 32 && character <= 126 &&
                    ...
                    if (name_field_active && valid_name && active_input.size() < 16)
                        ...
                        !name_field_active && room_code_character(static_cast<unsigned char>(character)) &&
                        active_input.size() < 6
                    )
                        active_input += static_cast<char>(std::toupper(static_cast<unsigned char>(character)));
                    character = GetCharPressed();
                }
            }

            const bool create_requested = IsKeyPressed(KEY_F2) || (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) &&
            ...
            if (create_requested && !online_attempt_pending) {
                ...
                begin_online_attempt([&online_client, name] {
                    ...
                    if (!room) return OnlineAttempt{false, "Could not reach the relay. Try again in a moment."};
                    if (!online_client.connect(*room, name))
                        return OnlineAttempt{false, "Room created, but its WebSocket could not connect."};
                    return OnlineAttempt{true, "Joining room " + *room + "..."};
                });
            }
            const bool join_requested =
                (!name_field_active && IsKeyPressed(KEY_ENTER)) ||
                (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && CheckCollisionPointRec(mouse, lobby_join_button));
            if (join_requested && room_input.size() == 6 && !online_attempt_pending) {
                ...
                player_name_input = sanitize_player_name(player_name_input);
                const std::string name = player_name_input.empty() ? "Player" : player_name_input;
                const std::string token = room == online_room && !online_token.empty() ? online_token
                                          : room == saved_session.room && name == saved_session.name
                                              ? saved_session.token
                                              : "";
                begin_online_attempt([&online_client, room, name, token] {
                    const bool connected = online_client.connect(room, name, token);
                    return OnlineAttempt{
                        connected,
                        connected ? "Joining room " + room + "..." : "Room not found or unavailable. Check the code."
                    };
                });
            } else if (join_requested && room_input.size() != 6) {
                online_status = "A room code has exactly six characters.";
            }
        } else if (!sound_panel && !reaction_panel && !replay_active && !falling.active && !game.over()) {
            if (IsKeyPressed(KEY_A) || IsKeyPressed(KEY_LEFT)) selected_column = std::max(0, selected_column - 1);
            if (IsKeyPressed(KEY_D) || IsKeyPressed(KEY_RIGHT)) selected_column = std::min(6, selected_column + 1);
            auto play_column = [&](int column) {
                if (online_mode) {
                    if (!rival_connected || online_attempt_pending)
                        ...
                        online_status =
                            online_seat ? "Waiting for the other player." : "Spectators cannot place discs.";
                } else
                    start_drop(column);
            };
        ...
        // change-detection keeps the condition true until the cooldown allows
        // the send, so the final column is always delivered.
        cursor_send_cooldown = std::max(0.0F, cursor_send_cooldown - dt);
        if (online_mode && rival_connected && !online_lobby && !online_attempt_pending && !falling.active &&
            !game.over() && cursor_send_cooldown <= 0 && online_seat == static_cast<int>(game.current()) &&
        ...
        if (falling.active && (!replay_active || !replay_paused)) {
            ...
            if (falling.y >= target) {
                ...
                if (falling.bounces == 0 && falling.velocity > 260) {
                    ...
                    falling.velocity = 0;
                    if (replay_active) {
                        replay_board[falling.move.row * connect4::columns + falling.move.column] = falling.move.player;
                        ++replay_index;
                        replay_delay = 0.2F;
        ...
        const bool replay_complete = replay_active && replay_index >= replay_source.size() && !falling.active;
        ...
        // --- Drawing: background, header badge, match panel ---

        BeginDrawing();
        const bool round_revealed = game.over() && !falling.active && pending_falls.empty() && outcome_announced;
        DrawRectangleGradientV(0, 0, screen_width, screen_height, Color{13, 18, 33, 255}, background);
        ...
        if (!round_revealed || !game.draw()) {
            ...
            text(bold, focus_label.c_str(), 132, 528, focus_label.size() > 15 ? 17 : 22, player_color(focus_player));
        } else {
        ...
        const bool show_opponent_aim = online_mode && opponent_column >= 0 && opponent_cursor_seat != online_seat &&
        ...
        for (int row = 0; row < connect4::rows; ++row)
            for (int col = 0; col < connect4::columns; ++col) {
                ...
                DrawCircle(static_cast<int>(x + 2), static_cast<int>(y + 4), 33, Color{0, 0, 0, 105});
                DrawCircle(static_cast<int>(x), static_cast<int>(y), 31, empty_hole);
                DrawCircleLines(static_cast<int>(x), static_cast<int>(y), 31, Color{255, 255, 255, 15});
                const Player player =
                    replay_active ? replay_board[row * connect4::columns + col] : game.grid()[row][col];
                const bool is_falling_target = falling.active && falling.move.row == row && falling.move.column == col;
        ...
        const int display_winner = replay_active ? replay_model.winner : static_cast<int>(game.winner());
        const bool show_win_line =
            (!replay_active && round_revealed && display_winner != 0) || (replay_complete && display_winner != 0);
        if (show_win_line) {
            const auto& wins = replay_active ? replay_model.winning_cells : game.winning_cells();
            const Vector2 start{board_x + wins[0][1] * cell + cell / 2, board_y + wins[0][0] * cell + cell / 2};
            const Vector2 target{board_x + wins[3][1] * cell + cell / 2, board_y + wins[3][0] * cell + cell / 2};
            const float eased = win_line_progress * win_line_progress * (3 - 2 * win_line_progress);
            const Vector2 end{start.x + (target.x - start.x) * eased, start.y + (target.y - start.y) * eased};
            ...
            if (win_line_progress >= 1) {
                ...
                const Vector2 shine{start.x + (target.x - start.x) * sweep, start.y + (target.y - start.y) * sweep};
        ...
        // --- Drawing: banners, footer text and overlays ---
        ...
        if (round_revealed && !replay_active) {
            ...
            const std::string replay_title =
                "Replay  " + std::to_string(replay_index) + " / " + std::to_string(replay_source.size());
            const Vector2 bounds = MeasureTextEx(bold, replay_title.c_str(), 25, 0);
            const float center = board_x + board_width / 2;
            DrawRectangleRounded({center - bounds.x / 2 - 20, 78, bounds.x + 40, 45}, .45F, 16, Color{18, 24, 39, 248});
